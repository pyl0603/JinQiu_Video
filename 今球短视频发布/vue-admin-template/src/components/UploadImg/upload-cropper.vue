  ...
  <div class="wrapper">
    ...
    <div class="content">
      <div class="show-info">
        ...
        />
        <button @click="finish2()" class="btn">裁剪并提交</button>
      </div>
    </div>
  </div>
</template>
...
axios.defaults.headers.common["Authorization"] = "Bearer" + " " + getCategory();
...
export default {
  ...
  mounted() {
      this.example2.fixedNumber = [this.widthNum,this.heightNum]
  },
  methods: {
    //点击裁剪，这一步是可以拿到处理后的地址
    finish2() {
      this.$refs.cropper2.getCropData(data => {
        ...
        return new Promise((resolve, reject) => {
          axios({
            url: 'https://api.jinqiulive.com/client-admin/upload/picture',
            // url: '/dev-api/client-admin/upload/picture',
            method: "post",
            data: form,
            headers: { "Content-Type": "multipart/form-data" }
          }).then(res => {
            this.$message("上传成功");
            this.$emit('cropper',res.data)
            // resolve(res.data);
          });
        });
      });
