<template>
  <div class="article mt10">
    <el-row>
      ...
      <el-col :span="24">
        <el-col :span="10">
          ...
          <el-col class="mt30">
            <em>文章摘要</em>：
            <el-input v-model="articleCont.subhead"></el-input>
          </el-col>
        </el-col>
        <el-col :span="14">
          <el-row class="up-img mb20">
            ...
            <el-col :span="8">
              <div class="upload-before" v-if="articleCont.imageUrl == ''" @click="isPopImg = true">
                <svg-icon icon-class="tupian"></svg-icon>
                <span class="f14 cada">690*390</span>
              </div>
              <div class="upload-after" v-if="articleCont.imageUrl != ''" @click="isPopImg = true">
          ...
          <el-row class="up-video mt30">
            <el-row class="mb20">
              视频(是否置顶)：
              <el-switch :value="articleCont.videoLocation" @change="changeVideoPos" />
            </el-row>
            <el-col :span="8">
              ...
              <!-- <uploadV ref="uploadV" @upVideo="upVideo"></uploadV> -->
            </el-col>
          </el-row>
        </el-col>
    ...
    ></imgPop>
    <!-- 富文本图片选择 -->
    <imgManyPop ref="imgManyPop" @cancel="isPopImgMany = false"
      v-if="isPopImgMany" @chooseItem="chooseItemMany"></imgManyPop>
    <videoPop
      ref="videoPop"
    ...
    <div class="mt20" v-if="contStatus === 3 || contStatus === 2">
      ...
      <div class="el-col-12 tar">
        <el-button @click="artAtatus === 'add' ? addArticles(3) : putArticleGo(3)" class="mr30">保存</el-button>
        <el-button
          type="dqx-btn"
          @click="artAtatus === 'add' ? addArticles(0) : putArticleGo(0)"
          class="ml30"
        >保存并提审</el-button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    // 保存修改并发布
    async publishPut(){
      let res = await this.putArticles(this.contStatus);
      if(res === 0){
        this.$message("修改成功");
        this.$router.push({
          path: "/edit/contMana",
    ...
    async checked() {
      let res = await this.putArticles(this.contStatus);
      if(res === 0){
      contCheck(this.$route.query.id, 1, "article").then(_ => {
        this.$message("审核通过");
        this.$router.push({
          path: "/edit/contMana",
          query: { type: "article" }
    ...
    // 驳回
    refuseCheck() {
      contCheck(this.$route.query.id, 2, "article").then(_ => {
        this.$message("操作成功");
        this.$router.push({
          path: "/edit/contMana",
          query: { type: "article" }
    ...
    // 视频位置
    changeVideoPos(val) {
      this.articleCont.videoLocation = !this.articleCont.videoLocation;
    },
    // 是否自动发布
    changeAuto(val) {
    ...
    addArticles(val) {
      ...
      let videoLocation = 0;
      if (this.articleCont.videoUrl != "") {
        videoLocation = this.articleCont.videoLocation ? 1 : 2;
      }
      this.articleCont.content = this.articleCont.content + '<p><strong>本文若有抄袭、侵权、违规等情况，请与今球官方联系。转载请注明本文转自今球</strong></p>'
      addArticle({
    ...
    // 修改文章调用方法
    async putArticleGo(val){
      let res = await this.putArticles(val);
      if(res === 0){
          this.$message("修改成功");
          this.$router.push({
          path: "/edit/contMana",
