  ...
  <div class="banner">
    <el-row>
      ...
      <el-col :span="15" :push="2">
        ...
        <div class="mt20 mb20 cada f16">Banner标准尺寸为750*420，以帮助你快速调整Banner展示效果。*可实时预览您上传图片的展示效果*</div>
        <!-- banner列表 -->
          ...
          <el-col :span="16" :push="1">
            <div>
              <div class="mb10 banner-top">
                ...
                <em class="cp el-col-6 banner-cont f14" @click="getCont(index)">{{ item.type | typeTxt}}</em>
                <span class="el-col-6 mb20 cada ml20" v-show="item.isMain">默认开启</span>
                <el-switch class="el-col-4 ml20" v-model="item.on" @change="changeSwitch" v-show="!item.isMain"/>
                <em class="el-col-3 cp c698" v-if="index === 2||index === 3||index === 4" @click="setTop(index)">置顶</em>
              </div>
              <div class="choose-cont">
                <div class="el-col-15">
                  <el-input type="textarea" v-model="item.summary" v-inputRule maxlength="24"></el-input>
                  <!-- <input class="cada f14 mt10">{{item.summary == '' ? '空：请选择您所需要关联的内容' : item.summary}} -->
                </div>
              </div>
            </div>
          </el-col>
    ...
    <choose-cont ref="chooseCont" :showConn="showConn" @matchId="matchId" :isMain="isMain" :isUrl="true" @closePop='showConn = false'></choose-cont>
    <!-- 图片选择器 -->
    <imgPop ref="imgPop" @cancel="cancelPop" v-if="isPopImg" @chooseItem="chooseItem" :isEdit="false"></imgPop>
  </div>
</template>
...
import { getBanner, putBanner } from "@/api/setting";
...
export default {
  ...
  methods: {
    ...
    confirm(){
      ...
      putBanner(this.bannerListInfo).then(_ => {
        this.$message("操作成功");
      })
    },
    // banner置顶
    ...
    matchId(val) {
      if(val.length === 3){
        this.bannerImg[this.chooseIndex].type = val[2];
        this.bannerImg[this.chooseIndex].url = val[1];
        this.bannerImg[this.chooseIndex].summary = val[1].substring(0,24);
      }else{
      this.bannerImg[this.chooseIndex].type = val[0];
      this.bannerImg[this.chooseIndex].summary = val[1].substring(0,24);
      this.bannerImg[this.chooseIndex].target = val[2];
      this.bannerImg[this.chooseIndex].url = null;
      }
      if(val.length === 5){
        this.bannerImg[this.chooseIndex].matchId = val[4];
      }else{
        this.bannerImg[this.chooseIndex].matchId = null;
      }
      this.showConn = false;
      console.log(this.bannerImg)
    },
      ...
      getBanner().then(res => {
        ...
        res.data.map(e => {
          this.bannerImg.push({
            ...
            type: e.type,
            image: e.image,
            summary: e.summary.substring(0,24),
            on: e.on,
            isMain: e.isMain,
            target: e.target,
            url:e.url,
