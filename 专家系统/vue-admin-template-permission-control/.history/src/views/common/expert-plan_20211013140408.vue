<template>
  <div class="match-item">
    ...
    >
      ...
      <el-table-column prop="author" label="作者" width="300">{{ scope.row.publisher.nickName }}</el-table-column>
...
export default {
  ...
  methods: {
    handleClick(val) {
      if(val.matchId){
        this.$emit("matchId", ["0", val.title, val.id, 0,val.matchId]);
      }else{
        this.$emit("matchId", ["0", val.title, val.id, 0]);
      }
