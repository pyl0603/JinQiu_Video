      ...
      <div className="phone">
        {
         ...
         ?
          this.props.preview.map((res,index) => {
            return (
              <div key={index}>
                <img key={index} src={res.url} style={{ width: res.x/2 - 18 + 'px', height: res.y/2 + 'px' }} />
              </div>
            )
          })
          :
          <div className="phone-content">
            {
              ...
              this.props.preview.map((res,index) => {
                return (
                  ...
                  <div key={index}>
                    <span>{this.props.reviewTxt}</span>
                    <img className={res.name === '直播暂停' ? 'live-pause': null} src={res.url} style={{ width: res.x/2 + 'px', height: res.y/2 + 'px' }} />
                  </div>
                  :
