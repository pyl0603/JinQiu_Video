<template>
  <div class="video">
    ...
    <el-row class="tar">
      <el-input placeholder="请输入内容" v-model="keyworlds" maxlength="50" v-inputRule class="input-with-select">
    ...
    >
      ...
      <el-table-column label="发布时间" width="200">
        <template slot-scope="scope">{{ (scope.row.publishTime || scope.row.createTime) | formatTime }}</template>
      </el-table-column>
    ...
    <el-dialog title="发布预览" :visible.sync="dialogVisible" :show-close="true">
      ...
      <div class="cont-all" :class="{half:isHis}">
        <div class="f18 cover-img">
          文章封面：
          <img :src="contInfo.cover" alt />
        </div>
        <div class="c171 f18 mt10">
          频道：
          <span v-for="item in contInfo.channels" :key="item.id">{{item.channelName}} <i v-if="index != contInfo.channels.length - 1">,</i></span>
        </div>
        <div class="c171 f20 mt20">{{contInfo.title}}</div>
        <div class="cada f16 mt10">{{contInfo.publishTime | formatTime}}</div>
        <video :src="contInfo.video" controls="controls" class="mt10" v-if="contInfo.video"></video>
        <div class="mt10 f16 c171 cont-det" v-html="contInfo.content"></div>
      </div>
