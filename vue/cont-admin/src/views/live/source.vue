      ...
      <el-table-column label="功能" width="150">
        <template slot-scope="scope">
          <el-button
                ...
                >广告链接</el-button
              >
          <el-button size="mini" type="text" @click="setSource(scope.row.id)">直播源设置</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 弹窗 -->
    <el-dialog title="直播源设置" :visible.sync="dialogVisibleSource" width="30%" :show-close="false"  :close-on-click-modal="false">
      <div class="dialog-cont">
        ...
        <div slot="footer" class="dialog-footer mt20 tac">
          ...
          <el-button
            type="dqx-btn"
            :disabled="isAdd"
            @click="dialogStatusSource==='create'?createSourceData():updateSourceData()"
          >保存</el-button>
...
export default {
  ...
  methods: {
    ...
    addList() {
      let nullNo = 0;
      this.listSource.map(res => {
        if (res.name == "" || res.url === "") {
          nullNo++;
        }
      });
      if (nullNo > 0) return;
      this.listSource.push({ url: "", name: "" });
    },
    // 频道切换
    tabChoose(val, e) {
      this.itemIndex = val;
