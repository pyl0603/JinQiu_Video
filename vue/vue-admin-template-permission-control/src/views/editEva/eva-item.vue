<template>
  ...
  <div class="right el-col-7">
    ...
    <div class="eva-add" v-if="contId != 0">
      <el-input type="textarea" placeholder="评论内容" v-inputRule @keyup.enter.native="addEvas" v-model="cont"></el-input>
      <div class="send-btn">
        当前用户名：{{evaName}}
...
export default {
  ...
  methods: {
    ...
    async getIndex() {
      ...
      getEva({ page: this.page, target: this.contId, type: type }).then(res => {
        this.list = res.data;
  ...
  computed: {
    dataList() {
      return [this.contId, this.typeName, this.page, this.isRefresh];
    },
