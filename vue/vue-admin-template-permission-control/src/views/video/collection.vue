<template>
  <div>
    ...
    >
      ...
      <el-table-column label="合集简介" prop="note"> </el-table-column>
      <el-table-column label="合集名称" prop="collectionName">
      </el-table-column>
      <el-table-column label="视频数量" prop="videoCount"> </el-table-column>
...
import { getVideo, failVideo, passVideo, deleteVideo,getVideoStatistical,
collectionCheckList,
collectionCheckPass,
collectionCheckFail,
...
export default {
  ...
  methods: {
    async handlePass(index, row) {
        const res = await collectionCheckPass(row.id);
      if (res.code === 200) {
        this.$message({
          message: "合集审核通过",
          type: "success"
    ...
    async handleFail(index, row) {
            const res = await collectionCheckFail(row.id);
      if (res.code === 200) {
        this.$message({
          message: "合集审核不通过",
          type: "success"
    ...
    async getVideoList() {
      ...
      const res = await collectionCheckList(this.getVideoOptions)
      if (res.code === 200) {
