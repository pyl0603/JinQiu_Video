...
import chooseCont from "@/components/Dialog/expert";
import { getBanner, putBanner } from "@/api/expert";
  ...
  methods: {
    ...
    associated(val) {
      if(val[0] === '3') {
        ...
        this.bannerImg[this.chooseIndex].target = 0;
      } else if(val[0] === '4') {
        this.bannerImg[this.chooseIndex].type = val[0];
        this.bannerImg[this.chooseIndex].summary = val[1];
        this.bannerImg[this.chooseIndex].target = val[2];
      }
      console.log(val,this.bannerImg)
      this.showConn = false;
    },
    // 获取banner列表
    getList() {
