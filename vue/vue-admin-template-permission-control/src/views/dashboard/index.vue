...
export default {
  data() {
    ...
    return {
      ...
      tableData: [], //列表
      start: new Date(new Date().setHours(0, 0, 0, 0)).getTime(), //开始时间
      end:
        new Date(new Date().setHours(0, 0, 0, 0)).getTime(), //结束时间
      pickerOptions: {
        shortcuts: [
          {
            text: "今天",
      ...
      getCount(
        {
          type: 1,
          page: this.page,
          start: parseInt(this.start - 3600 * 1000 * 2.5),
          end: parseInt(this.end + 3600 * 1000 * 21.5) 
        },
        type
      ).then(res => {
