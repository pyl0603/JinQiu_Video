<template>
  <div>
        ...
        <div class="avatar-container">
          <div class="avatar-wrapper">
            <div class="mr60 cp f16">
              <span
                class="mr20 choose-style"
                @click="checkType('Right', 'left')"
                :class="{ on: isEva }"
                >← 去评论</span
              >
              <span
                class="choose-style"
                @click="checkType('otherRight', 'otherLeft')"
                :class="{ on: !isEva }"
                >去发帖 →</span
              >
            </div>
    ...
    ></left>
    <!-- 中间 -->
    <div class="center el-col-9">
      <!-- 文章详情 -->
      <el-col :span="18" :push="3" v-if="cont != ''">
        <div class="title f24 c171 mt10">{{ cont.title }}</div>
        <div class="cada f14 mt10">
          <span class="mr20">{{ cont.author }}</span>
          {{ cont.updateTime | formatTime }}
        </div>
      ...
      <el-col :span="18" :push="3" v-if="types === 'match'&&matchCont.home">
        <div class="match-name">{{ matchCont.home.name }}VS{{ matchCont.away.name }}</div>
        <div class="match-time">{{ matchCont.matchTime | formatTime }}</div>
      </el-col>
      <!-- 帖子详情 -->
      ...
      <div class="child" @click.stop.prevent="isChange = true">
        ...
        <div class="list">
          <div class="item" v-for="item in childList" :key="item.id">
            ...
            <el-button type="text" @click.stop.prevent="setNewId(item)"
              >切换</el-button
            >
          </div>
        </div>
        <div class="new-add">
          <em>新增子账户：</em>
          <el-input placeholder="请输入昵称" v-model="niceNameAdd"></el-input>
          <el-button type="dqx-btn" @click.stop.prevent="addNewChild"
            >确定</el-button
          >
        </div>
      </div>
      ...
      <div class="child" @click.stop.prevent="isMsg = true">
        <div class="list">
          <div
            ...
            :key="item.id"
            @click="detEva(item.rootId)"
          >
            <div class="reply-time-item">
              <i>{{ item.author.nickname }}</i
              >回复<i class="c18c">你</i>：{{ item.content }}
            </div>
    ...
    >
      <div class="main-eva f16 main-area">
        ...
        <div class="eva-right ml20">
          ...
          <div class="eva-conts mt10">{{ mainEva.content }}</div>
        </div>
      </div>
      ...
      <div class="eva-list">
        <div
          class="main-eva f16"
          v-for="item in replyEvaList"
          :key="item.id"
          @click="replyReply(item)"
        >
          ...
          <div class="eva-right ml20">
            <div class="eva-info">
              <span
                ...
                }}<em v-if="item.refer.avatar"
                  >回复：{{ item.refer.nickname }}</em
                ></span
              >
      ...
      <div class="add-reply">
        <el-input
          type="textarea"
          :placeholder="remindTxt"
          v-model="replyCont"
          @keyup.enter.native="addReply"
        ></el-input>
        <el-button type="dqx-btn" class="send-btn" @click="addReply" size="mini"
          >发送</el-button
        >
      </div>
    </el-dialog>
...
import { setId, getCategory, setCategory } from "@/utils/eva-auth";
...
export default {
  ...
  methods: {
    ...
    addReply() {
      let type;
      if (this.types === "article") type = 0;
      if (this.types === "video") type = 1;
      if (this.types === "circle") type = 4;
      let id = this.id === 0 ? this.disIds : this.id;
      addEva({
        type: type,
        target: id,
  ...
  mounted() {
    ...
    this.isBt = getCategory() == 0 ? false : true;
    this.getChilds();
  },
  watch: {
    id(newVal) {
...
.change-type {
  ...
  width: 163px;
  height: 42px;
  background: url("~img/img_home_football.png") no-repeat center;
  background-size: 100%;
}
.change-type.on {
  background: url("~img/img_home_basketball.png") no-repeat center;
  background-size: 100%;
}
.choose-style.on {
  padding-bottom: 5px;
